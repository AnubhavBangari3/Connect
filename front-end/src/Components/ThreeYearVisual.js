...
function ThreeYearVisual() {
  ...
  useEffect(()=>{
    ...
    const fetchData = async () => {
      try {
        const response = await fetch(`http://127.0.0.1:8000/lifeplan/getvision3year/${params.id}`, {
          method: 'GET',
          headers: {
        ...
        const anyInactive = jsonData.some((item) => !item.three_year);
        setform(anyInactive);
  ...
  const handleSubmit = (event) => {
    ...
    fetch(`http://127.0.0.1:8000/lifeplan/vision3year/${params.id}`, {
      method: 'POST',
      body: formData,
      headers: {
  ...
  const handleGeneratePDF = () => {
    fetch(`http://127.0.0.1:8000/lifeplan/generate-three-year-vision-pdf/${params.id}`, {
      method: 'GET',
      headers: {
        'Authorization': `Bearer ${token["access_token"]}`
      }
  ...
  return (
    <div>
        ...
        <div>
          ...
          <Document>
            <Page>
              ...
              <View>
                <Text style={styles.heading}>Three year Vision Board</Text>
                {data.map((item, index) => (
                    <View key={item.id}>
      ...
      {pdfUrl && (
        ...
        } fileName="three_year_vision.pdf">
...
export default ThreeYearVisual
