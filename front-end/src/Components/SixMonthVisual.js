...
function SixMonthVisual() {
  ...
  useEffect(()=>{
    ...
    const fetchData = async () => {
      try {
        const response = await fetch(`http://127.0.0.1:8000/lifeplan/getvision6month/${params.id}`, {
          method: 'GET',
          headers: {
        ...
        const anyInactive = jsonData.some((item) => !item.six_month);
        setform(anyInactive);
  ...
  const handleSubmit = (event) => {
    ...
    fetch(`http://127.0.0.1:8000/lifeplan/vision6month/${params.id}`, {
      method: 'POST',
      body: formData,
      headers: {
  ...
  const handleGeneratePDF = () => {
    fetch(`http://127.0.0.1:8000/lifeplan/generate-six-month-vision-pdf/${params.id}`, {
      method: 'GET',
      headers: {
        'Authorization': `Bearer ${token["access_token"]}`
      }
        ...
        <div>
          ...
          <Document>
            <Page>
              ...
              <View>
                <Text style={styles.heading}>Six Month Vision Board</Text>
                {data.map((item, index) => (
                    <View key={item.id}>
