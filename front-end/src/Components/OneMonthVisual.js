 ...
 {/* code for pdf start */}
  ...
  useEffect(()=>{
    ...
    const fetchData = async () => {
      try {
        const response = await fetch(`http://127.0.0.1:8000/lifeplan/getvision1month/${params.id}`, {
          method: 'GET',
          headers: {
        ...
        const anyInactive = jsonData.some((item) => !item.one_month);
        setform(anyInactive);
  ...
  const handleSubmit = (event) => {
    ...
    fetch(`http://127.0.0.1:8000/lifeplan/vision1month/${params.id}`, {
      method: 'POST',
      body: formData,
      headers: {
  ...
  const handleGeneratePDF = () => {
    fetch(`http://127.0.0.1:8000/lifeplan/generate-one-month-vision-pdf/${params.id}`, {
      method: 'GET',
      headers: {
        'Authorization': `Bearer ${token["access_token"]}`
      }
          ...
          {data.length >0 && (
            ...
            <Document>
              <Page>
                ...
                <View>
                  <Text style={styles.heading}>One Month Vision Board</Text>
                  {data.map((item, index) => (
                      <View key={item.id}>
