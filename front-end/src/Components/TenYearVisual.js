...
function TenYearVisual() {
  ...
  useEffect(()=>{
    ...
    const fetchData = async () => {
      try {
        const response = await fetch(`http://127.0.0.1:8000/lifeplan/getvision10year/${params.id}`, {
          method: 'GET',
          headers: {
        ...
        const anyInactive = jsonData.some((item) => !item.ten_year);
        setform(anyInactive);
  ...
  const handleSubmit = (event) => {
    ...
    fetch(`http://127.0.0.1:8000/lifeplan/vision10year/${params.id}`, {
      method: 'POST',
      body: formData,
      headers: {
  ...
  const handleGeneratePDF = () => {
    fetch(`http://127.0.0.1:8000/lifeplan/generate-ten-year-vision-pdf/${params.id}`, {
      method: 'GET',
      headers: {
        'Authorization': `Bearer ${token["access_token"]}`
      }
        ...
        <div>
          ...
          <Document>
            <Page>
              ...
              <View>
                <Text style={styles.heading}>Ten Year Vision Board</Text>
                {data.map((item, index) => (
                    <View key={item.id}>
